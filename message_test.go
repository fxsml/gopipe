...
func TestMessage_MultiStageAcking_TwoStages(t *testing.T) {
	t.Parallel()
...
func TestMessage_MultiStageAcking_ThreeStages(t *testing.T) {
	t.Parallel()
	...
	// Set expected ack count to 3
	if !msg.SetExpectedAckCount(3) {
		t.Fatal("Expected SetExpectedAckCount to succeed")
	}

	// First two acks should not trigger callback
	for i := 0; i < 2; i++ {
		if !msg.Ack() {
			t.Errorf("Expected Ack %d to return true", i+1)
		}

		if ackCalled {
			t.Errorf("Expected ack callback NOT to be called after ack %d", i+1)
		}
	}

	// Third ack should trigger callback
	if !msg.Ack() {
		t.Error("Expected third Ack to return true")
	}

	if !ackCalled {
		t.Error("Expected ack callback to be called after third ack")
	}
}

func TestMessage_MultiStageAcking_ExtraAcks(t *testing.T) {
	t.Parallel()
	...
	if ackCount != 1 {
		t.Errorf("Expected ack callback to be called exactly once, got %d", ackCount)
	}
}

func TestMessage_MultiStageAcking_NackBlocksAcks(t *testing.T) {
	t.Parallel()
	...
	// Further acks should fail
	if msg.Ack() {
		t.Error("Expected Ack after Nack to return false")
	}

	if ackCalled {
		t.Error("Expected ack callback NOT to be called after nack")
	}
}

func TestMessage_MultiStageAcking_ConcurrentAcks(t *testing.T) {
	t.Parallel()
	...
	// Set expected ack count to 10
	if !msg.SetExpectedAckCount(10) {
		t.Fatal("Expected SetExpectedAckCount to succeed")
	}

	// Ack concurrently 20 times
	const goroutines = 20
	var wg sync.WaitGroup
	wg.Add(goroutines)
...
func TestMessage_MultiStageAcking_SetCountAfterAcking(t *testing.T) {
	t.Parallel()
	...
	// Try to set expected ack count after acking started
	if msg.SetExpectedAckCount(5) {
		t.Error("Expected SetExpectedAckCount to return false after acking started")
	}
}

func TestMessage_MultiStageAcking_InvalidCount(t *testing.T) {
	t.Parallel()
	...
	// Try to set zero count
	if msg.SetExpectedAckCount(0) {
		t.Error("Expected SetExpectedAckCount(0) to return false")
	}

	// Try to set negative count
	if msg.SetExpectedAckCount(-1) {
		t.Error("Expected SetExpectedAckCount(-1) to return false")
	}
}

func TestMessage_MultiStageAcking_CopyMessage(t *testing.T) {
	t.Parallel()
	...
	// Copy the message
	copy := gopipe.CopyMessage(msg, "copied")

	// Ack on the copy should complete the sequence
	if !copy.Ack() {
		t.Error("Expected Ack on copy to return true")
	}

	if !ackCalled {
		t.Error("Expected ack callback to be called after acking copy")
	}

	// Both original and copy should report as acked
	if !msg.Ack() {
		t.Error("Expected original to report as acked")
	}

	if !copy.Ack() {
		t.Error("Expected copy to report as acked")
	}
}

func TestMessage_MultiStageAcking_BroadcastScenario(t *testing.T) {
	t.Parallel()

	// Simulate a broadcast scenario where a message is sent to 3 processors
	var ackCalled bool
	var mu sync.Mutex
	...
	// Set expected ack count to 3 (for 3 processors)
	if !msg.SetExpectedAckCount(3) {
		t.Fatal("Expected SetExpectedAckCount to succeed")
	}

	// Create 3 copies (simulating broadcast)
	copy1 := gopipe.CopyMessage(msg, "copy1")
	copy2 := gopipe.CopyMessage(msg, "copy2")
	copy3 := gopipe.CopyMessage(msg, "copy3")

	// Process concurrently (simulating parallel processors)
	var wg sync.WaitGroup
	wg.Add(3)
	...
	go func() {
		...
		if !copy1.Ack() {
			t.Error("Expected copy1 Ack to succeed")
		}
	}()
	...
	go func() {
		...
		if !copy2.Ack() {
			t.Error("Expected copy2 Ack to succeed")
		}
	}()
	...
	go func() {
		...
		if !copy3.Ack() {
			t.Error("Expected copy3 Ack to succeed")
		}
	}()

	wg.Wait()

	// Verify ack was called exactly once after all processors completed
	mu.Lock()
	defer mu.Unlock()

	if !ackCalled {
		t.Error("Expected ack callback to be called after all processors acked")
	}
}

func TestMessage_MultiStageAcking_BroadcastWithOneFailure(t *testing.T) {
	t.Parallel()

	// Simulate a broadcast where one processor fails
	var ackCalled, nackCalled bool
	var mu sync.Mutex
	...
	// Process concurrently with one failure
	var wg sync.WaitGroup
	wg.Add(3)
	...
	go func() {
		defer wg.Done()
		time.Sleep(20 * time.Millisecond) // Simulate processing
		copy2.Nack(errors.New("processor 2 failed"))
	}()
	...
	// Verify nack was called and ack was not
	mu.Lock()
	defer mu.Unlock()

	if !nackCalled {
		t.Error("Expected nack callback to be called after one processor failed")
	}
