...
func TestSubscriber_ContextCancel(t *testing.T) {
	...
	t.Run("cancel waits for in-flight requests", func(t *testing.T) {
		...
		go func() {
			msg, ok := <-ch
			if !ok {
				return // Channel closed before message received
			}
...
// TestSubscriber_ConcurrentRequestsDuringShutdown tests the WaitGroup race fix.
// This test runs ServeHTTP concurrently while canceling the context to ensure
// the WaitGroup.Add happens atomically with the subscribed check.
func TestSubscriber_ConcurrentRequestsDuringShutdown(t *testing.T) {
	// Run this test multiple times to increase chance of detecting races
	for i := 0; i < 10; i++ {
		sub := NewSubscriber(SubscriberConfig{AckTimeout: 100 * time.Millisecond})
		ctx, cancel := context.WithCancel(context.Background())
		ch, _ := sub.Subscribe(ctx)
		...
		for j := 0; j < 20; j++ {
			go func() {
				body := []byte(`{"specversion":"1.0","id":"test","type":"test","source":"/test","data":{}}`)
				req := httptest.NewRequest(http.MethodPost, "/events", bytes.NewReader(body))
				req.Header.Set("Content-Type", ContentTypeCloudEventsJSON)
				w := httptest.NewRecorder()
				sub.ServeHTTP(w, req)
				// Don't check status - may be 200 or 503 depending on timing
				done <- struct{}{}
			}()
		}
