...
//
// Subscribe can only be called once. ServeHTTP is safe for concurrent use.
...
func (s *Subscriber) Subscribe(ctx context.Context) (<-chan *message.RawMessage, error) {
	...
	go func() {
		...
		// Mark as unsubscribed to prevent new requests from entering.
		// Must happen before closing done and calling Wait to avoid race.
		s.mu.Lock()
		s.subscribed = false
		s.mu.Unlock()
...
func (s *Subscriber) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Check if subscribed and track request atomically to avoid WaitGroup race.
	// Must hold lock across both operations: if we check subscribed, release lock,
	// then Add(1), the shutdown goroutine could call Wait() between those steps.
	s.mu.RLock()
	if !s.subscribed {
		s.mu.RUnlock()
	...
	s.wg.Add(1) // Add while holding lock
	s.mu.RUnlock()
	defer s.wg.Done()
