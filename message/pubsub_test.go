...
func TestPublisher_Basic(t *testing.T) {
	...
	go func() {
		...
		msgs <- message.New([]byte("msg2"), message.WithSubject[[]byte]("topic-a"))
		msgs <- message.New([]byte("msg3"), message.WithSubject[[]byte]("topic-b"))
	}()
...
func TestPublisher_Concurrency(t *testing.T) {
	...
	go func() {
		defer close(msgs)
		for i := 0; i < 10; i++ {
			msgs <- message.New([]byte("msg"), message.WithSubject[[]byte]("topic-a"))
		}
	}()
