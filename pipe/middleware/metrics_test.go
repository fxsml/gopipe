...
func TestMetricsMiddleware_Basic(t *testing.T) {
	var got []*Metrics
	...
	metricsMw := MetricsMiddleware[int, int](collector)
	wrappedFunc := metricsMw(processFunc)

	_, _ = wrappedFunc(context.Background(), 5)
