...
func TestBroker_MultipleSubscribers(t *testing.T) {
	b := memory.NewBroker(memory.Config{BufferSize: 10})
...
func TestBroker_SendTimeout(t *testing.T) {
	b := memory.NewBroker(memory.Config{SendTimeout: 10 * time.Millisecond})
...
func TestBroker_BatchSend(t *testing.T) {
	b := memory.NewBroker(memory.Config{BufferSize: 100})
